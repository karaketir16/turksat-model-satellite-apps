...
//  qDebug() << "LSM9DS0 WHO_AM_I's returned: " << status << "Should be <<" <<0x49D4;
...
//uint32_t millis()
//{
//    uint32_t ms = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::high_resolution_clock::
//                  now().time_since_epoch()).count();
//    return ms;
//}
...
//void loop()
//{
//  if( 1 /*digitalRead(DRDYG)*/) {  // When new gyro data is ready
...
//  if(1 /*digitalRead(INT1XM)*/) {  // When new accelerometer data is ready
...
//  if(1 /*digitalRead(INT2XM)*/) {  // When new magnetometer data is ready
...
//    qDebug() << "ax = " << (int)1000*ax << " ay = " << (int)1000*ay  << " az = " << (int)1000*az << " mg";
//    qDebug() << "gx = " << gx << " gy = " <<  gy << " gz = " << gz <<" deg/s";
//    qDebug() << "mx = " << (int)1000*mx << " my = " << (int)1000*my << " mz = " << (int)1000*mz << " mG";

//    qDebug() << "temperature = " << temperature;

//    qDebug() << "Yaw, Pitch, Roll: " << yaw << ", " << pitch << ", " << roll;
//    qDebug() << "q0 = " << q[0] << " qx = " << q[1] << " qy = " << q[2] << " qz = " << q[3];

//    qDebug() << "filter rate = " << 1.0f/deltat;
////    display.clearDisplay();
